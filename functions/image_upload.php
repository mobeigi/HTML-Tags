...
if (isset($_POST['submitbtn'])) {
    ...
    for ($i = 0; $i < count($_imagefileS['imagefile']['name']); $i++) {//loop to get individual element from the array

        $validextensions = array("jpeg", "jpg", "png");  //Extensions which are allowed
        $ext = explode('.', basename($_imagefileS['imagefile']['name'][$i]));//explode imagefile name from dot(.) 
	  ...
	  if (in_array($imagefile_extension, $validextensions)) {
            if (move_uploaded_imagefile($_imagefileS['imagefile']['tmp_name'][$i], $target_path)) {//if imagefile moved to uploads folder
